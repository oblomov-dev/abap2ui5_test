CLASS z2ui5_cl_http_handler DEFINITION
  ...
  PUBLIC SECTION.
      ...
      BEGIN OF ty_s_client,
        o_body   TYPE REF TO zcl_2ui5_hlp_tree_json,
...
METHOD main_roundtrip.
    ...
    DO.
      ...
      TRY.
          ROLLBACK WORK.
          lo_server->ms_db-i_app->on_event( NEW lcl_2ui5_client( lo_server )  ).
          ROLLBACK WORK.

        CATCH cx_root INTO DATA(cx).
          lo_server = lo_server->init_new_server(
            zzcl_app_system=>factory_error(  server = lo_server error = cx app = lo_server->ms_db-i_app kind = 'ON_EVENT' ) ).
      ...
      TRY.
          ...
          lo_server->mo_ui5_model = zcl_2ui5_hlp_tree_json=>factory( ).
          lo_server->mo_view_model = lo_server->mo_ui5_model->add_attribute_object( 'oViewModel' ).

          ROLLBACK WORK.
          lo_server->ms_db-i_app->set_view( NEW lcl_2ui5_view( lo_server ) ).
          ROLLBACK WORK.

        CATCH cx_root INTO cx.
          lo_server = lo_server->init_new_server(
            zzcl_app_system=>factory_error( server = lo_server error = cx app = lo_server->ms_db-i_app kind = 'SET_SCREEN' )
             ).
          CONTINUE.
      ENDTRY.
